...
export default function ModalDeleteUser({ open, setOpen, id, token, dict }: Props) {
    const router = useRouter();
    const { notificationOnChange, actionOnChange, typeNotificationOnChange } = useAuthContext();

    const handleDeleteUser = async () => {
        actionOnChange(Actions.DELETE_ACCOUNT);
        try {
                            ...
                            <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                                ...
                                <div className="sm:flex sm:items-start">
                                    ...
                                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                        <Dialog.Title
                                            ...
                                            {dict?.profile?.delete?.modal?.title}
                                        </Dialog.Title>
                                        <div className="mt-2">
                                            <p className="text-sm text-gray-500">
                                                {dict?.profile?.delete?.modal?.description}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                                    <button
                                        ...
                                        {dict?.profile?.delete?.modal?.delete}
                                    </button>
                                    <button
                                        type="button"
                                        className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto"
                                        onClick={() => setOpen(false)}>
                                        {dict?.profile?.delete?.modal?.cancel}
                                    </button>
                                </div>
                            </Dialog.Panel>
                        </Transition.Child>
